...
//        });
    ...
    private void handleWakeLockAcquire(XC_MethodHook.MethodHookParam param, String wakeLockName, IBinder lock, int uId) {
        ...
        boolean block = mLockFreePrefs.getBoolean(prefName, false);//获取标志位
        int overrideSeconds = -1;//超时时间?
        if (!block) {//如果不阻止
            //See if there is a wildcard block on this//正则表达式
            Set<String> wakelockWildcards = mLockFreePrefs.getStringSet("wakelock_regex_set", null);
            if (wakelockWildcards != null) {//正则不为空
                ...
                for (String wildcard : wakelockWildcards) {
                    ...
                    if (wakeLockName.matches(wildcard)) {
                        block = true;//如果符合正则 设置block
                        overrideSeconds = Integer.parseInt(wildcard.substring(wildcard.indexOf("$$||$$") + 6));//延迟??
        ...
        if (block) {
            ...
            if (overrideSeconds != -1) {
                ...
                collectorMaxFreq = mLockFreePrefs.getLong("wakelock_" + wakeLockName + "_seconds", 240);//加延时?
            }
            collectorMaxFreq *= 1000; //convert to ms

            //Debounce this to our minimum interval.//反转到最低
            long lastAttempt = 0;
            try {
                lastAttempt = mLastWakelockAttempts.get(trackingWakelockName);//上一次允许这个唤醒锁的时间点
            } catch (NullPointerException npe) { /* ok.  Just havent attempted yet.  Use 0 */ }

            long now = SystemClock.elapsedRealtime();//现在时间
            long timeSinceLastWakeLock = now - lastAttempt;//距离上次允许时间过了多久
            ...
            if (timeSinceLastWakeLock < collectorMaxFreq) {
                ...
                mLastWakelockAttempts.put(trackingWakelockName, now);//记录现在时间
                recordAcquire(wakeLockName, lock, uId);//记录唤醒锁信息
            }
        } else {
...
//        }
    ...
    private void recordAcquire(String wakeLockName, IBinder lock, int uId) {
        ...
        if (curStats == null) {
            ...
            curStats.setUId(uId);
            curStats.setTimeStarted(SystemClock.elapsedRealtime());//开始时间
...
//    }
    ...
    private void handleWakeLockRelease(XC_MethodHook.MethodHookParam param, IBinder lock) {
        InterimEvent curStats = mCurrentWakeLocks.remove(lock);//移除唤醒锁
        if (curStats != null) {
            curStats.setTimeStopped(SystemClock.elapsedRealtime());
            Context context = (Context) XposedHelpers.getObjectField(param.thisObject, "mContext");
