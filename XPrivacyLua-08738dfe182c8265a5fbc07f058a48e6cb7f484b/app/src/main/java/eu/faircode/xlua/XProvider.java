...
class XProvider {
    ...
    static Cursor query(Context context, String method, String[] selection) throws RemoteException {
        loadData(context);//准备数据库

        Cursor result = null;
        StrictMode.ThreadPolicy originalPolicy = StrictMode.getThreadPolicy();//线程严苛模式
        try {
            StrictMode.allowThreadDiskReads();//忽略磁盘读
            StrictMode.allowThreadDiskWrites();//忽略磁盘写
            switch (method) {
                case "getHooks":
    ...
    private static Cursor getAssignedHooks(Context context, String[] selection, boolean marshall) throws Throwable {
        ...
        MatrixCursor result = new MatrixCursor(new String[]{marshall ? "blob" : "json", "used"});//虚拟表
    ...
    private static void loadHooks(Context context) throws Throwable {
        ...
        // Read built-in definition 内置的定义?
        PackageManager pm = context.getPackageManager();
        ...
        // Read external definitions  外部定义
        dbLock.readLock().lock();
        try {
            db.beginTransaction();
            try {
